    ...
    <div class="container">
      <div class="row">
        <div class="col-12 col-md-8 col-lg-8 pt-5 text-center">
          ...
          <div v-if="account">
            <h3 class="mb-5">Update metadata</h3>
            <input
              class="text-center"
              ...
              v-model="metadata"
            /><br /><br />
            <div class="btn mint-btn" @click="updateBaseURI()">
              Update base URI
            </div>
          </div>
        </div>
      ...
      if (parseInt(netId) !== app.networks[app.network]) {
        ...
        if (accounts.length > 0) {
          ...
          if (app.account !== owner) {
            ...
            const uri = await nftContract.methods.contract_base_uri().call();
            app.printLog("Base URI is: " + uri);
            // Specific for Test
          }
        }
      }
    ...
    async updateBaseURI() {
      ...
      try {
        ...
        const receipt = await nftContract.methods
          .fixURIs(0, app.metadata)
          .send({ from: app.account })
          .on("transactionHash", (tx) => {
            app.printLog("Waiting for confirmation at: " + tx);
          });
        app.printLog("URI updated correctly");
